...
rm prod.json
pnpm install --prod --filter "backend-auth-node" --filter "backend-fintech-mongo" --filter "backend-fintech-mongo-cron" --filter "kafka-mongo" --filter "grpc-auth-node" --filter "grpc-fintech-node" --filter "graphql-utils" --filter "grpc-utils" --filter "jwt-utils" --filter "kafka-utils" --filter "logs-utils" --filter "mongo-utils" --filter "redis-utils" --filter "utils" && (MONGO_DB=$MONGO_DB REFRESHSECRET=$REFRESHSECRET ACCESSSECRET=$ACCESSSECRET REDIS=$REDIS NODE_ENV=$NODE_ENV KAFKA=$KAFKA KAFKA_USERNAME=$KAFKA_USERNAME KAFKA_PASSWORD=$KAFKA_PASSWORD GRPC_AUTH=$GRPC_AUTH GRPC_FINTECH=$GRPC_FINTECH AWS_MAIN_KEY=$AWS_MAIN_KEY AWS_MAIN_SECRET=$AWS_MAIN_SECRET AWS_REGION=$AWS_REGION pnpm serve > /dev/null 2> /dev/null < /dev/null &)