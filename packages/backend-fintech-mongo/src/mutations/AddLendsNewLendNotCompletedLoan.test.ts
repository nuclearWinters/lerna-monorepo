...
describe("AddLends one lend and loan no completed goal", async () => {
  ...
  grpcServer.bindAsync("0.0.0.0:1985", ServerCredentials.createInsecure(), (err) => {
    if (err) {
      return;
    }
  });
  const grpcClient = new AuthClient("0.0.0.0:1985", credentials.createInsecure());
