...
describe("AddLends one lend and loan no completed goal", async () => {
  ...
  grpcServer.bindAsync("0.0.0.0:1984", ServerCredentials.createInsecure(), (err) => {
    if (err) {
      return;
    }
  });
  const grpcClient = new AuthClient("0.0.0.0:1984", credentials.createInsecure());
  ...
  it("test AddLends valid access token", { timeout: 10_000 }, async () => {
    ...
    strictEqual(recordsResult.length, 6);
    deepStrictEqual(
