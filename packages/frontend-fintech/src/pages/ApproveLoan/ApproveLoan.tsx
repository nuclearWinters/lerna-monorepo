...
import { authUserQuery } from "Routes";

const approveLoansFragment = graphql`
  query ApproveLoansQuery {
    user {
      id
      ...ApproveLoans_user
    }
...
const subscriptionApproveLoans = graphql`
  subscription ApproveLoansSubscription($connections: [ID!]!) {
...
const ApproveLoansPaginationFragment = graphql`
  fragment ApproveLoans_user on User
  @argumentDefinitions(
    count: { type: "Int", defaultValue: 5 }
    cursor: { type: "String", defaultValue: "" }
  )
  @refetchable(queryName: "ApproveLoansPaginationUser") {
    approveLoans(first: $count, after: $cursor)
      @connection(key: "ApproveLoans_user_approveLoans") {
      edges {
        node {
          id
          ...LoanRow_loan
...
export const ApproveLoans: FC<Props> = (props) => {
  ...
  >(ApproveLoansPaginationFragment, user);
