...
import { ACCESS_TOKEN_EXP_NUMBER, base64Name, jwt, REFRESH_TOKEN_EXP_NUMBER } from "../utils";
...
describe("RevokeSessionMutation tests", () => {
    ...
    it("RevokeSessionMutation: is another user", async () => {
        ...
        const otherRefreshTokenExpireTime = new Date().getTime() / 1000 + REFRESH_TOKEN_EXP_NUMBER;
        ...
        expect(isBefore(data.data.revokeSession.session.expirationDate, new Date())).toBeTruthy();
    });
